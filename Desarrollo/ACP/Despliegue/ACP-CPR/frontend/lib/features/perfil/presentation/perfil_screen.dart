/// Pantalla de Perfil.
/// Solo "Cerrar sesión" es funcional. Las demás opciones son visuales.
...
class _PerfilScreenState extends ConsumerState<PerfilScreen> {
  ...
  Future<void> _cargarPerfil() async {
    try {
      ...
      final resp = await dio.get('/perfil');
      setState(() {
        _nombre = resp.data['full_name'] ?? '';
        _email = resp.data['email'] ?? '';
        _cargando = false;
      });
    } catch (e) {
      // Obtener email de la sesión de Supabase
      final sesion = Supabase.instance.client.auth.currentSession;
      setState(() {
        _email = sesion?.user.email ?? '';
        _nombre = 'Usuario';
        _cargando = false;
      });
    }
